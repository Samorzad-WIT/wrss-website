...
await Promise.all(
  pairs.map(async ({ id, url }) => {
    ...
    try {
      ...
      if (status !== 200) {
        console.warn(`  ⚠  event ${id}: HTTP ${status}`)
        return
      }

      const ogImageUrl = extractOgImage(html)
      if (!ogImageUrl) {
        console.warn(`  ⚠  event ${id}: brak og:image`)
        return
      }
...
console.log(
  `✓ fetch-events: zapisano ${Object.keys(banners).length} banerów → src/data/events-banners.ts`,
)
